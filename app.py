...
def profile():
    ...
    if user_made.count() > 0:
        ...
        args = {
            ...
            "recipes_sorted": user_recipes.find({"$and":
                                                [{"username": user},
                                                 {'_id': {'$gte': last_id}}]})
                                          .sort('_id', pymongo.ASCENDING)
                                          .limit(pagination_limit),
            "next_url": '/profile?limit=' + str(pagination_limit) +
                        '&offset=' + str(pagination_offset + pagination_limit),
            "prev_url": '/profile?limit=' + str(pagination_limit) +
                        '&offset=' + str(pagination_offset - pagination_limit),
            "recipes": user_made,
...
def recipes():
    ...
    args = {
        ...
        "recipes_sorted": recipe_collection.find({'_id': {'$gte': last_id}})
                                           .sort('_id', pymongo.ASCENDING)
                                           .limit(pagination_limit),
        "next_url": '/recipes?limit=' + str(pagination_limit) +
                    '&offset=' + str(pagination_offset + pagination_limit),
        "prev_url": '/recipes?limit=' + str(pagination_limit) +
                    '&offset=' + str(pagination_offset - pagination_limit),
        "recipes": recipes,
        "total_results": total_results
    }
...
def filter_recipes():
    ...
    starting_id = recipe_collection.aggregate([{"$match":
    ...
    if results_count != 0:
        ...
        args = {
            "limit": pagination_limit,
            "offset": pagination_offset,
            "recipes_sorted": recipe_collection.aggregate([{"$match":
                                                           {"$and":
                                                            [{"$and": get_results(form)},
                                                             {"_id": {"$gte": last_id}}]}},
                                                          {"$sort": {"_id": 1}},
...
def search_recipes():
    ...
    if results_count != 0 and search_text is not None:
        ...
        args = {
            ...
            "next_url": '/search_recipes?limit=' + str(pagination_limit) +
                        '&offset=' + str(pagination_offset + pagination_limit),
            "prev_url": '/search_recipes?limit=' + str(pagination_limit) +
                        '&offset=' + str(pagination_offset - pagination_limit),
            "total_results": total_results
        }
...
def search_results(search_input):
    if request.method == 'POST':
        result = recipe_collection.find({"$text":
                                         {"$search": str(search_input)}})
...
def update_rating(recipe_id):
    if request.method == 'POST':
        ...
        instance_record = ratings_collection.find_one({"user_id": user_id,
                                                       "recipe_id": recipe_number})
        if instance_record is not None:
            ratings_collection.update({"user_id": user_id,
                                       "recipe_id": recipe_number},
                                      {"$set": {"rating": rating}})
